...
__device__ ${s_ctype} get_F_mermin(${c_ctype} *sample)
{
    ...
    %for i in xrange(particles):
    product1 = product1 * (sigma_x[${i}] + make_${c_ctype}(0, 1) * sigma_y[${i}]);
    product2 = product2 * (sigma_x[${i}] + make_${c_ctype}(0, -1) * sigma_y[${i}]);
    %endfor

    return 0.5 * (product1 - product2).y;
}
...
__device__ ${s_ctype} get_F_ardehali(${c_ctype} *sample)
{
    ...
    sigma_type sigma_a = (sigma_x[${particles-1}] +
        sigma_y[${particles-1}]) * ${1 / numpy.sqrt(2)};
    sigma_type sigma_b = (-sigma_x[${particles-1}] +
        sigma_y[${particles-1}]) * ${1 / numpy.sqrt(2)};
    ...
    return -(product * (sigma_a - sigma_b)).x + (product * (sigma_a + sigma_b)).y;
}
